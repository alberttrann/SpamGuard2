...
def classify_with_lm_studio(client: OpenAI, message: str, model_name: str) -> dict:
    ...
    try:
        ...
        if prediction_text not in ['ham', 'spam']:
            print(f"\nWarning: Model returned an invalid response: '{prediction_text}'. Defaulting to 'ham'.")
            prediction_text = 'ham'

    except Exception as e:
        print(f"\nERROR: An API call to LM Studio failed: {e}. Is the server running?")
...
def evaluate_lm_studio_model(model_name: str):
    """
    Main function to run the full evaluation suite against a model in LM Studio.
    """
    ...
    try:
        ...
        print(f"❌ FATAL ERROR: Could not connect to LM Studio server at {LM_STUDIO_BASE_URL}.")
        print("Please ensure you have loaded a model and clicked 'Start Server' in the LM Studio app.")
        return
    ...
    print("\n" + "="*60)
    print(f"  DETAILED REPORT FOR: {model_name} (via LM Studio)")
    print("="*60)
    ...
    plt.title(f'LM Studio - {model_name} Confusion Matrix')
    plt.ylabel('True Label')
    plt.xlabel('Predicted Label')
    plt.show()

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate a model running in LM Studio.")
    parser.add_argument("model_name", type=str, help="The identifier for the model you are testing (e.g., 'Phi-3-mini-4k-instruct').")
    args = parser.parse_args()
    
    evaluate_lm_studio_model(args.model_name)
